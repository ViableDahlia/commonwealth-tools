# Commonwealth Tools

A collection of lightweight, browser-based tools for **Fallout 4 modders** built around settlement building and mod management.

No installs. No accounts. No data sent anywhere except where you choose.

🔗 **[Open the tools](https://viabledahlia.github.io/commonwealth-tools/)**

---

## Tools

### [MO2 Mod List Reader](./mo2-modlist/)
Read and explore your Mod Organizer 2 mod list directly in the browser. Drag in your `modlist.txt` to search, filter, and sort your full mod list by name, priority, or status - without opening MO2.

**[Open tool →](https://viabledahlia.github.io/commonwealth-tools/mo2-modlist/)**

---

### [Nexus Tracker Enhanced](./nexus-tracker-enhanced/)
Browse your NexusMods tracked mod list in the browser. Connect with your personal API key to search by name or author, filter by game, and identify mods that have been removed or hidden.

**[Open tool →](https://viabledahlia.github.io/commonwealth-tools/nexus-tracker-enhanced/)**

> ⚠️ **NexusMods API registration in progress.** This tool is currently in the testing phase while awaiting registration with the NexusMods team as required by the [API Acceptable Use Policy](https://help.nexusmods.com/article/114-api-acceptable-use-policy). In the meantime it is available for personal use and testing with your own personal API key.

---

## About

Made by **Viable Dahlia**, a Fallout 4 settlement builder focused on architectural approaches to in-game construction, inspired by [Gemma Wheeler Architecture](https://youtube.com/@gemmawheeler).

- [Nexus Mods Profile](https://www.nexusmods.com/profile/viabledahlia)

---

## License

Code in this repository is licensed on a per-tool basis.

| Tool | License |
|---|---|
| MO2 Mod List Reader | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) |
| Nexus Tracker Enhanced | [MIT](./nexus-tracker-enhanced/LICENSE) |

See each tool's folder for its individual licence file.
